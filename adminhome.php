			...
			<div class="row">
				<div class="col-lg-12 p-b-30">
					<div class="t-center">
						...
						<?php
$Query = mysql_query("SELECT * FROM issues");
if(mysql_num_rows($Query)>0){
	echo "<h2>PROBLEM DETECTED</h2>";
?>
						</h3>
					</div>
...
<thead>
	<tr>
    	<th>Sl No</th><th>CPU NO</th><th>MOUSE NO</font></th>
		<th>MONITOR NO</th><th>KEYBOARD NO</th><th>OS</th>
		<th>GROUP NAME</th><th>PROBLEM</th><th>WITH IN WARRENTY</th>
	</tr>
</thead>
<tbody>
<?php  
 ...
 $pid=$row['pid'];
 $Q = mysql_query("SELECT p.warrenty,p.supplier FROM pctable p where p.id='$pid'");
 if($r=mysql_fetch_array($Q)){ 
 $warrenty=$r['warrenty'];
 $supplier=$r['supplier'];
 $_SESSION['sname'] = $supplier;
 ?>
	...
	<tr>
    <td><?php echo $sl?></td>
    	<td><?php echo $row['cpuno']?></td>
        <td><?php echo $row['mouseno']?></td>
        <td><?php echo $row['monitorno']?></td>
        <td><?php echo $row['keyboardno']?></td>
        <td><?php echo $row['os']?></td>
        <td><?php echo $row['gname']?></td>
        <td><?php echo $row['comment']?></td>
		<td><?php if( date("Y-m-d H:i:s")>$warrenty)
		{
		$_SESSION['probid'] = $row['id'];
		?><a href="servicec.php">NO CONTACT SERVICE CENTER</a>
		<?php
		}else {
		$_SESSION['probid'] = $row['id'];
		?><a href="damagedpc.php">YES CONTACT SUPPLIER</a><?php }?>
		</td> 
        </tr>
